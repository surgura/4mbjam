...
bool DrawNodeEditorWindow(
	LSystem::Plant* plant, OperationDatabase* id_db, ed::EditorContext* context)
{
	bool anything_changed = false;
			...
			if (dragged_from && dragged_to) // If both are valid link was dragged from and to a pin
			{
				...
				if (ed::AcceptNewItem())
				{
					...
					if (id_db->IsInputID(dragged_from.Get()) && id_db->IsOutputID(dragged_to.Get()))
					{
						...
						anything_changed |=
							plant->AddConnection({output_op, output_index, input_op, input_index});
					}
					else if (
						id_db->IsInputID(dragged_to.Get()) && id_db->IsOutputID(dragged_from.Get()))
					{
...
void UpdateCameraState(Camera& camera, CameraState& camera_state, EditorConfig& editor_config)
{
	...
	if (GetMouseWheelMove() != 0)
	{
		const float desired_distance =
			camera_state.camera_distance * std::pow(0.95, GetMouseWheelMove());
	...
	if (editor_config.orbit_mode)
	{
		...
		camera_state.camera_rotation_updown = LSystem::MapRange<float>(
			updown, -1, 1, editor_config.camera_min_y, editor_config.camera_max_y);
	}
	else
	{
		// Manual controls:
	...
	const Quaternion sideways_quaternion =
		QuaternionFromEuler(0, camera_state.camera_rotation_sideways, 0);
	const Vector3 pos_xz =
		Vector3RotateByQuaternion({0, 0, camera_state.camera_distance}, sideways_quaternion);
	const Vector3 tangent =
		Vector3RotateByQuaternion(pos_xz, QuaternionFromEuler(0, -glm::half_pi<float>(), 0));
	const Quaternion updown_quaternion =
		QuaternionFromAxisAngle(tangent, camera_state.camera_rotation_updown);
...
int main()
{
	...
	while (!WindowShouldClose())
	{
		...
		ClearBackground(Color(
			editor_config.background_color.r * 255,
			editor_config.background_color.g * 255,
			editor_config.background_color.b * 255,
			255));
		...
		{
			DrawLine3D(
				ToVec3(l.point_a.position), ToVec3(l.point_b.position), ToColor(l.point_b.color));
		}
		...
		if (changed)
		{
			...
			if (!plant_vertices.triangles.empty())
			{
				...
				auto position_data =
					new glm::vec3[plant_vertices.triangles.size() * 3]; // Cleaned by raylib
				auto color_data = new std::
					array<uint8_t, 4>[plant_vertices.triangles.size() * 3]; // Cleaned by raylib
