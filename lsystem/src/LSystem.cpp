...
std::vector<glm::vec4>
TransformCircle(const std::vector<glm::vec4>& circle, const glm::mat4& transform)
{
	...
	int remaining_recursions)
{
	...
	if (instruction->data->draw_branch)
	{
		...
		else if (cascading_state.branch_sides > 1)
		{
			...
			{
				int j = i + 1 == cascading_state.branch_sides
							? 0
							: i + 1; // Take next index or 0 if were at end
	...
	for (const auto& child : instruction->data->children)
	{
		...
		if (child)
		{
			...
			{
				ExecuteInstruction(
					child, branch_transform, cascading_state, vertex_buffer, remaining_recursions);
			}
		}
	}
}
...
VertexBuffer InstructionPool::Generate(int recursions)
{
	...
	{
		ExecuteInstruction(
			first_instruction, glm::mat4(1), cascading_state, render_buffer, recursions);
	}
