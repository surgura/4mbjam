...
CreatePhyllotaxis(InstructionPool& lsystem, int count, float length, float spread, float roll)
{
	...
	if (count > 0)
	{
		...
		{
			instructions.push_back(lsystem.CreateExtrusion(
				length, roll + i * golden_angle, pitch_ratio * glm::sqrt(static_cast<float>(i))));
		}
	}
...
PhyllotaxisOperation::PhyllotaxisOperation() : Operation({1, 1, "Create Phyllotaxis"})
{
...
void PhyllotaxisOperation::Execute(
	...
	if (branch_count > 0)
	{
		...
		for (auto onto : active_input_values)
		{
			auto new_instructions =
				CreatePhyllotaxis(lsystem, branch_count, branch_length, spread, roll);
			onto->data->children.insert(
				onto->data->children.end(), new_instructions.begin(), new_instructions.end());
			instructions.insert(
