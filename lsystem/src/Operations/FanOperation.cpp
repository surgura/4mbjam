...
CreateFan(InstructionPool& lsystem, int count, float length, float spread, float roll)
{
	...
	if (count > 0)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			instructions.push_back(
				lsystem.CreateExtrusion(length, roll, angle_start + i * angle_increment));
		}
	}
...
FanOperation::FanOperation() : Operation({1, 1, "Create Fan"})
{
