...
RandomColorOperation::RandomColorOperation() : Operation({1, 0, "Apply Random Color"})
{
	AddParameter(color);
...
void RandomColorOperation::Execute(
	...
	Plant* plant)
{
	...
	if (!is_nan.x && !is_nan.y && !is_nan.z)
	{
		...
		{
			auto new_hsv = glm::clamp(
				glm::gaussRand(hsv, deviation), {0.0f, 0.0f, 0.0f}, {360.0f, 1.0f, 1.0f});
