...
CreateFork(InstructionPool& lsystem, int count, float length, float spread, float roll)
{
	...
	if (count > 0)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			instructions.push_back(
				lsystem.CreateExtrusion(length, roll + i * angle_increment, spread / 2));
		}
	}
...
ForkOperation::ForkOperation() : Operation({1, 1, "Create Fork"})
{
