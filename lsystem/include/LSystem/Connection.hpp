...
struct Connection
{
	...
	Connection() : output(), input(), output_index(-1), input_index(-1) { }
...
struct hash<LSystem::Connection>
{
	std::size_t operator()(const LSystem::Connection& c) const
	{
		size_t hash_value = 0;
		LSystem::AddToHash(
			hash_value, std::hash<LSystem::Identifier<LSystem::Operation>>()(c.output));
		LSystem::AddToHash(
			hash_value, std::hash<LSystem::Identifier<LSystem::Operation>>()(c.input));
